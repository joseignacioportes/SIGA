...
$sql_extraer_categoria_RES=$sql_extraer_categoria_QRY->fetch(PDO::FETCH_COLUMN);

$ticket_reasignacion_categoria="
SELECT  Id_Subcategoria,
        Desc_Subcategoria
FROM    siga_cat_ticket_subcategoria
WHERE   Id_Categoria=$sql_extraer_categoria_RES
";
...
$ticket_reasignacion_categoria_array=[];
    ...
    for ($i=0; $i < count($ticket_reasignacion_categoria_res) ; $i++) {    
        if($sql_extraer_categoria_RES[$i]['Id_Subcategoria'] == $sql_extraer_categoria_RES){$selected="selected";}else{$selected="";}

            $ticket_reasignacion_categoria_array[]="<option value='".$ticket_reasignacion_categoria_res[$i]['Id_Subcategoria']."' $selected>".$ticket_reasignacion_categoria_res[$i]['Desc_Subcategoria']."</option>";
    }
