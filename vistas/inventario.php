...
include_once(dirname(__FILE__) . "/../modelos/simple_mvc/ActivoFijoInventarioReportesModel.Class.php");
	...
	$centroDeCostosAssist = $inventarioClass->getCentrosDecostos();
...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
	...
	<div class="row" style="<?php if($Id_Menu==25){echo"display:none"; } ?>">
		<div class="col-md-3 col-sm-6 col-xs-12">
			<div class="info-box azul">
				...
				<a href="#" data-toggle="modal" data-target="#altaEquipo" data-id-tabla="tablaactivos" onclick="limpiarcampos(); limpiarcamposproveedor(); limpiarcamposcontabilidad(); cargarAdminAccesoriosActivos();">
		...
		<div class="col-md-3 col-sm-6 col-xs-12" id="div_btn_reasignar_activo">
			<div class="info-box negro">
				<a href="#reasignacion" aria-controls="reasignacion" role="tab" data-toggle="tab" id="tabReasignacion_activo"
				onclick="$('#tiposInventario li').removeClass('active');">
	...
	<!-- ==== Pestañas del Inventario por Status ==== -->
	<div class="row">
		<ul id="tiposInventario" class="nav nav-tabs azulf" role="tablist">
			<li role="presentation" style="<?php if($Id_Menu==25){echo"display:none";}?>" class="active">
				<a href="#inventario" data-id-tabla="tablaactivos" aria-controls="inventario" role="tab" data-toggle="tab">Inventario</a>
			</li>
			<li role="presentation" style="<?php if($Id_Menu==25){echo"display:none";}?>">
				<a href="#bajas" id="tabEnProceso" data-id-tabla="tablebajas" data-estatus-baja="0" aria-controls="bajas" role="tab" data-toggle="tab" onclick="$('#Estatus_baja').val(0); generarTablaEncabezados(this);">En proceso de baja</a>
			</li>
			...
			<li role="presentation" style="<?php if($Id_Menu==25){echo"display:none";}?>"><a href="#reubicacion" data-id-tabla="tablereubicacion" aria-controls="reubicacion" role="tab" data-toggle="tab" onclick="generarTablaEncabezados(this);" id="tabreubicacion_activo">Reubicación</a>
			</li>
									...
									<tbody>
										...
										<tr>
											<td colspan="7" align="center"><br>
												<button type="button" class="btn chs" id="buscar_baja">Buscar</button>&nbsp;&nbsp;&nbsp;
												<button type="button" class="btn chs export" id="exportar_baja">
												<i class="fa fa-file-excel-o" aria-hidden="true"></i> Exportar a Excel</a></li></button><br>
										</tr>
									</tbody>
								</table>
								<!-- ==== Tabla de elementos ==== -->
								<div id="lstFiltros_tablebajas"></div>
								<div id="area_tablebajas">
									<h4 class="text-center">Cargando la estructura del reporte de Activos en Procesos de Baja / Baja definitiva</h4>
								</div>
							</div>
						</div>
					</div>
							...
							<div>
								...
								<table border="0" cellspacing="5" cellpadding="5" style="margin: 0 auto; margin-bottom: 1em;">
									<tbody>
										...
										<td align="center">
											<button type="button" class="btn chs" id="buscar_reubicacion" onclick="generarInventario_tablereubicacion()">Buscar</button><br>
										</tr>
									</tbody>
								</table>

								<div id="lstFiltros_tablereubicacion"></div>
								<div id="area_tablereubicacion">
									<h4 class="text-center">Cargando la estructura del reporte de Activos en Reubicación</h4>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="gray">
					...
					<script type="text/javascript">
						...
						function obtenerListaActivos() {
							var parametrosJson = { Id_Area: $("#idareasesion").val(), accion: "ActivosGet", "SoloInventarioUsable": 1 };
	...
	<!-- ==== Archivos Include Formularios de Edición de Activos en Ventanas Modal ==== -->
		<?php include("include_alta_activo.php") ?>
		<?php include("include_reubicacion_activo.php") ?>
		<?php include("include_baja_activo.php") ?>
		<?php include("activos/activo-workflow-modal.php") ?>
	<!-- ==== Modal para cargar la lista de accesorios y consumibles que están ligados a un Activo ==== -->
	...
	<link href="../plugins/datatables/extensions/ColReorder/css/dataTables.colReorder.css" type="text/css" rel="stylesheet" />
	<script type="text/javascript" src="../plugins/datatables/extensions/ColReorder/js/dataTables.colReorder.js"></script>
	-->
	...
	<script type="text/javascript">
		...
		$('#siga_activo_alta_fch_operacion').datepicker({
			format: 'dd/mm/yyyy',
			autoclose: true,
		}).datepicker();

		$('#siga_activo_alta_fch_recepcion').datepicker({
			format: 'dd/mm/yyyy',
			autoclose: true,
		}).datepicker();
		...
		function generarTablaEncabezados(elemento) {
			...
			jQuery("#area_" + NombreTabla).load("../controladores/simple_mvc/ActivoFijoInventarioReportesController.Class.php", strJsonParametros, function (response, status, xhr) {
				if (status == "error") {
					var msg = "Sorry but there was an error: ";
					mensajesalerta("Informaci&oacute;n", msg + xhr.status + " " + xhr.statusText, "error", "dark");
				}
				else {
					// La primera carga de las vistas es la de omisión y se llama por nombre sin versión del reporte definina,
		...
		$(document).ready(function(){
			//$('#centro_costos_assist_guardar').selectize();
			<?php
				// Llamada a la función que carga la estructura de tabla html con cada columna dinamica para cada uno de los reportes
				echo("generarTablaEncabezados('" . $lstTablasInventario[0] . "');");
				//for($i = 0; $i < count($lstTablasInventario); $i++) {
					// Genera la estructura html de cada tabla disponible para Inventarios
					//echo("generarTablaEncabezados('" . $lstTablasInventario[$i] . "'); ");
				//}
			?>
	...
	*/
    ...
    $nombrefuncion = "areas";
      ...
      //include ("includecombo.php");
			...
			// $nombrefuncion = "condicion";
			// $tabla = "siga_condicion_de_recepcion_estatus";
			// $id = "siga_condicion_de_recepcion_id";
			// $desc = "siga_condicion_de_recepcion_descripcion";
			// $nombre = "Condicion";
			// $excepto = "";
			// 	include ("includecombo.php");
	...
	$("#guardar").click(function () {
		...
		//Genera vale ed resguardo digital automatico
		...
		var strDatos 												= "";
		let siga_activo_alta_fch_operacion 	= $('#siga_activo_alta_fch_operacion').val();
		...
		if(Id_Tipo_Vale_Resg==2 && Area==2){
			sigaVrdAltaActivo(Num_Empleado);
		}
		...
		if (siga_activo_alta_fch_recepcion.length <= 0) {
			Agregar = false;
			mensaje_error += " -Falta agregar la fecha de recepción<br />";
			$("#siga_activo_alta_fch_operacion").focus();
		}

		if (siga_cmb_condicion_recepcion == 0) {
			Agregar = false;
			mensaje_error += " -Falta agregar la Condición de Recepción<br />";
			$("#siga_cmb_condicion_recepcion").focus();
		}
		//

		// if (siga_activo_alta_fch_operacion.length <= 0) {
		// 	Agregar = false;
		// 	mensaje_error += " -Falta agregar la fecha de entrada en operación<br />";
		// 	$("#siga_activo_alta_fch_operacion").focus();
		// }
			...
			$.ajax({
				...
				success: function (datos) {
					...
					if(Id_Activo.length <= 0)
					{
						...
						//limpiarcamposbaja();
						// Genera el inicio del WORKFLOW PARA ALTA
						generarWorkflow('tablaactivos');
						// Envía el correo a contabilidad
						//activaBoton(2);
	...
	$("#guardar2").click(function () {
	  ...
	  var VidaUtilCHS=$.trim($("#VidaUtilCHS").val());
		...
		if (Link.length <= 0) {
			Agregar = false;
			mensaje_error += " -Falta agregar Link del proveedor<br />";
		}
		...
		//if (!isNumber(MontoFactura_s_iva)) {
		//	Agregar = false;
		//	mensaje_error += " -El campo link es obligatorio<br />";
		//}
				...
				success: function (datos) {
					...
					if(json.totalCount > 0) {
						...
						$.ajax({
							...
							success: function (response) {
								...
								if($respuesta.intResultado < 0) {
									...
									mensajesalerta("Oh No! Accesorios y consumibles", $respuesta.strMensaje, "error", "dark");
								}
							},
							error: function () {
						...
						if(Id_activo_proveedor.length <= 0){
							...
							mensajesalerta("&Eacute;xito", "Actualizado Correctamente.2677", "success", "dark");
						}
		...
		if(Agregar)
		{
			...
			$.ajax({
				...
				success: function (datos) {
					...
					mensajesalerta("&Eacute;xito", "Actualizado Correctamente.2848", "success", "dark");
					//$('#altaEquipo').modal('hide');
					$('#tablaactivos').DataTable().ajax.reload();
  ...
  return str.length < max ? pad("0"+str, max) : str;
	...
	$("#AF_BC_reubicacion").change(function() {
		...
		$("#Id_UsuarioResponsableProcedencia").val("");
				...
				success: function (datos) {
					var data = "";
						...
						if (data.totalCount > 0) {
							...
							$("#cmbestatus_reubicacion").val(data.data[0].Id_Situacion_Activo);
			...
			$.ajax({
				...
				success: function (datos) {
					...
					if(json.totalCount > 0) {
						...
						$('#tablaactivos').DataTable().ajax.reload();
						$("#tabEnProceso").click();
						updateSituacionActivo(Id_Activo);
						//tablabaja(0);
					}
					else {
	...
	function updateSituacionActivo(Id_Activo){
		$.ajax({
			type: "POST",
			url: "/siga/class/admin/mesaDeAyuda/bajaActivo/bajaActivo.ajax.php",
			data: {accion:1, Id_Activo:Id_Activo},
			dataType: "JSON",
			success: function (response) {

			},
			error: function(){
				alert('SIGA: no se actualizó de manera adecuada la situacion del Activo.');
			}
		});
	...
	function enviaCorreoBaja(Id_Activo,Id_Baja,usuario,Paso) {
		...
		$.ajax({
			...
			success: function (datos) {
				if(datos==0){
      		mensajesalerta("SIGA:", "Correo enviado Correctamente.", "success", "dark");
      		$("#generarbaja").show();
				}else{
					mensajesalerta("SIGA:", "Ocurrio un error al enviar el correo de baja.", "error", "dark");
				}
			},
			error: function () {
				mensajesalerta("Oh No!", "Ocurrio un error al enviar el correo de baja.", "error", "dark");
			}
		});
	...
	function enviaCorreoAlta(Id_Activo,usuario)
	{
		$.ajax({
				...
				error: function () {
					mensajesalerta("Oh No!", "Ocurrio un error al enviar el correo a Contabilidad.", "error", "dark");
				}
			});
	}
...
alert('3733');
		...
		//var Centro_Costos_assist			= $.trim($("#centro_costos_assist_guardar").val());
		var cmbestatusreubicacionguar = $('#cmbestatusreubicacionguar').val();
		...
		var Centro_Costos_assist 			= $.trim($("#centro_costos_guardar").val());
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	  ...
	  /* Obligatorios */
			...
			strDatos += "&Centro_Costos="+Centro_Costos_assist;
			strDatos += "&Jefe_Area="+Jefe_Area;
...
if(Agregar){
	...
	$.ajax({
		type: "POST",
		url: "/siga/class/admin/mesaDeAyuda/reubicacion/reubicacion.ajax.php",
		data: {accion:1,Id_Usuario_Sesion:Id_Usuario_Sesion,Id_Activo:Id_Activo,Id_Area:Id_Area,Id_Ubic_Prim:Id_Ubic_Prim,Id_Ubic_Sec:Id_Ubic_Sec,Ubic_Especifica:Ubic_Especifica,Centro_Costos_assist:Centro_Costos_assist,
			cmbestatusreubicacionguar:cmbestatusreubicacionguar,Id_Usuario_Responsable:Id_Usuario_Responsable,Motivo_Reubicacion:Motivo_Reubicacion,Comentarios_Reubicacion:Comentarios_Reubicacion
		},
		dataType: "JSON",
		cache: false,
		beforeSend: function(){
			//generarreubicacion bloquear boton
		...
		success: function (response) {
			...
			$('#tablaactivos').DataTable().ajax.reload();
			generarInventario_tablereubicacion();
			//boton libre
			$("#generarreubicacion").prop("disabled", false);
			//console.log(response);
		},
		error: function (response){
			mensajesalerta("SIGA:", "Ocurrio un error operación de reubicación.", "error", "dark");
			$("#generarreubicacion").prop("disabled", false);
			//console.log(response);
			//boton libre
		}
			...
			// // Agrega la reubicación en donde quedó el activo
			// $.ajax({
			// 	type: "POST",
			// 	encoding:"UTF-8",
			...
			// 			$('#reubicacionEquipo').modal("hide");
			// 			mensajesalerta("&Eacute;xito", "Reubicacion guardada correctamente.", "success", "dark");
			// 			// Agrega el historico en donde estaba originalmente el Activo
			// 			actualizaReubicacion(Id_Activo,Id_Area,Id_Ubic_Prim,Id_Ubic_Sec,Ubic_Especifica,Centro_Costos,Id_Usuario_Sesion, Id_Usuario_Responsable, Nom_Usuario_Reponsable, responsable_procedencia, json.data[0].Id_Activo_Reubicacion, Id_UsuarioResponsableProcedencia);
			// 			// Recraga la tabla de Inventario Reubicación
			// 			$('#tablaactivos').DataTable().ajax.reload();
			// 			//
			// 			generarInventario_tablereubicacion();
			// 		}else{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	// Agrega el historico en donde estaba originalmente el Activo
	function actualizaReubicacion(Id_Activo,Id_Area,Id_Ubic_Prim,Id_Ubic_Sec,Ubic_Especifica,Centro_Costos,Id_Usuario_Sesion, Id_Usuario_Responsable, Nom_Usuario_Reponsable, Responsable_Procedencia, Id_Activo_Reubicacion, Id_UsuarioResponsableProcedencia) {
		var parametrosJson = { Id_Activo: Id_Activo,
			Id_Area: Id_Area, Id_Ubic_Prim: Id_Ubic_Prim, Id_Ubic_Sec : Id_Ubic_Sec,
			Ubic_Especifica: Ubic_Especifica, Centro_Costos: Centro_Costos, Id_Usuario_Sesion: Id_Usuario_Sesion, Id_Usuario_Responsable: Id_Usuario_Responsable,
			Nom_Usuario_Reponsable: Nom_Usuario_Reponsable, Responsable_Procedencia: Responsable_Procedencia, Id_Activo_Reubicacion: Id_Activo_Reubicacion,
			Id_UsuarioResponsableProcedencia: Id_UsuarioResponsableProcedencia
		};
		// console.log("ActualizaR reubicación: ");
		// console.log(parametrosJson);
		...
		$.ajax({
			...
			error: function () {
				mensajesalerta("Oh No!", "Ocurrio un error al enviar la reubicación.", "error", "dark");
			}
		});
	}
...
$('#fechaAlR').datepicker({
	...
	//alert('Fecha='+minDateFilter);
	//tablabaja.draw();
	generarInventario_tablereubicacion(); // Antes se llamaba tablareubicacion();
   });
...
*/
	...
	limpiarcampos = function() {
		...
		$('#siga_activo_alta_fch_operacion').val('');
		$('#siga_activo_alta_fch_recepcion').val('');
		...
		/* Si es biomedica */// Ajustes JGR 28/06/2018
		if ($("#idareasesion").val() == 1) {
			$("#cmbareas").val(1);
			$("#cmbclase").val(5);
			$("#cmbclase").change();
	...
	pasarvalores = function(id,estatus) {
		...
		if (id != "") {
			...
			$.ajax({
				...
				url: "../fachadas/activos/siga_activos/Siga_activosFacade.Class.php",
				async: false,
				data: { Id_Activo: id, accion: "consultar" },
				dataType: "html",
				beforeSend: function (xhr) { },
				success: function (data) {
					...
					if (data.totalCount > 0) {
						...
						format: 'dd/mm/yyyy',
						autoclose: true,
						}).datepicker('setDate',data.data[0].siga_activo_alta_fch_operacion);

						$('#siga_activo_alta_fch_recepcion').datepicker({
						format: 'dd/mm/yyyy',
						autoclose: true,
						}).datepicker('setDate', data.data[0].siga_activo_alta_fch_recepcion);

						sigaSelectCondicionDeRecepcion(id,data.data[0].siga_cmb_condicion_recepcion);
						...
						// Muestra la ventana modal con el formulario de Alta
	...
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			function validaWorkFlow(Id_Activo) {
				$.ajax({
					...
					error: function () {
						mensajesalerta("Oh No!", "Ocurrio un error al enviar (workflow).", "error", "dark");
					}
				});
			}
		...
		function activaBoton(tab) {
			// Oculta todos los botones de todas las secciones
			$("#guardar").hide(); $("#guardar2").hide(); $("#guardar3").hide();
			// Muestra el botón de la opción seleccionada
			if (tab == 1) { $("#guardar").show(); }
			else if (tab == 2) { $("#guardar2").show(); }
			else if (tab == 3) { $("#guardar3").show(); }
		}
		...
		function movimientoHorizontalBotonesExcel(IdTabla) {
			$("#" + IdTabla + "_wrapper div.dataTables_scrollBody").on("scroll", function (e) {
				...
				$("#" + IdTabla + "_wrapper .dataTables_scrollHead thead td button.btn-inicial-excel").attr("data-posicion-horizontal", -(horizontal));
			});
		}
		...
		function filtro_multiselect_generico(elementoSelect = null) {
			...
			var parametrosJson = { Id_Area: Id_Area, NombreTabla: Seccion, NombreCampo: NombreCampo, accion: "FiltroGenerico" };
			var lstFiltrosSuperior = new Array();
			//console.log("-->>" + IdTabla);
			//console.log("-->>" + Seccion);
			for(var i = 0; i < lstFiltrosXActualizar.length; i++) {
				...
				switch(lstFiltrosXActualizar[i]) {
					<?php
						// Recupera la lista de columnas administrables y que pueden usarse en las distintas versiones de los reportes
						// Las opciones "case" se obtienen completas en el código generado
						$parametrosConsulta = new ActivoFijoInventarioReportesModel();
						$listaColumnas = $parametrosConsulta->ActivoFilasColumnasCatalogoGet();
						for($i = 0; $i < count($listaColumnas); $i++) {
							?>case "#Filtro_<?php echo($listaColumnas[$i]->Nom_Columna); ?>_" + IdTabla:
								//console.log("parametrosJson.FiltroSuperior_<?php echo($listaColumnas[$i]->Nom_Columna); ?>");
								parametrosJson.FiltroSuperior_<?php echo($listaColumnas[$i]->Nom_Columna); ?> = optionSeleccionadas.join(",");
								lstFiltrosSuperior.push("<?php echo($listaColumnas[$i]->Nom_Columna); ?>");
								break;
							<?php
						}
					?>
			...
			// 1. Genera la consulta para formar los valores que conendrá el filtro tipo Excel
			// Agrega como parametro la lista de filtros que antecenden al filtro que será generado
			if(lstFiltrosSuperior.length > 0) {
				parametrosJson.lstFiltrosSuperior = lstFiltrosSuperior.join("|");
			}
			...
			resultado = cargo_cmb("../controladores/simple_mvc/ActivoFijoInventarioReportesController.Class.php", false, parametrosJson);
			//console.log(resultado);
			if(resultado.totalCount > 0) {
			...
			// 2. Genera el componente tipo Excel
			// Identificador del control select que fue rellenado para darle funcionalidad multiselect
			// Internamente recarga el datatable cuando se seleccione algunos de los elementos que se generaron en la consulta
			$(Id_Select_Filtro).multiselect({
				...
				disableIfEmpty: true,
				//includeFilterClearBtn: false,	// Elimina el botón de limpieza de filtros ya que existe el boton de selección de todos/ninguno
				templates: {
					filterClearBtn: '<span class="input-group-btn"><button title="Click para actualizar la información" class="btn btn-default multiselect-clear-filter" type="button" onclick="actualizarListaFiltro(\'' + Id_Select_Filtro + '\');"><i class="glyphicon glyphicon-search"></i></button></span>'
				},
				onChange: function() {
					...
					// Actualiza la lista de filtros y dispara la consulta para actualizar la tabla
				...
				onSelectAll: function() {
					...
					// Asigna el valor ala variable que será enviada para la actualización del Datatable
					$(Campo_Receptor).val(optionSeleccionadas.length > 0 ? "'" + optionSeleccionadas.join("','") + "'" : "");
					// Actualiza la Tabla correspondiente con las opciones seleccionadas
					//$("#" + IdTabla).DataTable().ajax.reload();
					// Actualiza la lista de filtros y dispara la consulta para actualizar la tabla
					//actualizarListaFiltro(Id_Select_Filtro);
				},
			...
			// Una vez generado el plugin, se muestra la lista de opciones que se cargaron en la consulta
			// Solo aplica cuando existan elementos encontrados en la consulta
			if(resultado.totalCount > 0) {
				setTimeout(function() {
					$(Id_Select_Filtro).parents("td").find('div.btn-group').addClass("open");

					// Determina la posición horizontal del botón [X] inicial para generar el filtro tipo Excel
					// Este parche es para abrir la lista de opciones una vez que se haya generado la consulta e inicializado el objeto multiselect
					// Cuando las columnas permanecen ocultas porque no caben en la pantalla (de manera horizontal), el plugin no ubica el control en la posición correcta y ya que se
					// forza a abirse de manera manual, se reposiciona el control en la misma posición horizontal del boton inicial
					var posicionHorizontalBtnMas = $(Id_Select_Filtro).parents("td").find("button.btn-inicial-excel").data("posicion-horizontal");
					if(parseInt(posicionHorizontalBtnMas) != 0) {
						//console.log($(Id_Select_Filtro).parents("td").find("button.btn-inicial-excel").data("posicion-horizontal"));
						$(Id_Select_Filtro).parents("td").find('div.btn-group').css("left", $(Id_Select_Filtro).parents("td").find("button.btn-inicial-excel").data("posicion-horizontal") + "px");
					}
				}, 100);
			}
		}
		...
		function actualizarListaFiltro(Id_Select_Filtro) {
			...
			// Vuelve a regenerar los combo select para actualizar los filtros tipo excel excepto los combos que se usan como filtros
			...
			for(var i = 0; i < lstFiltrosXActualizar.length; i++) {
				...
				//console.log($("#" + IdTabla + " th[data-id-select-filtro='" + lstFiltrosXActualizar[i].replace("#", "") + "']").data("nombre-campo"));
				lstFiltrosSuperior.push($("#" + IdTabla + " th[data-id-select-filtro='" + lstFiltrosXActualizar[i].replace("#", "") + "']").data("nombre-campo"));
			}
			...
			$("#listaFiltrosExcel_" + IdTabla).val((lstFiltrosSuperior.length > 0) ? lstFiltrosSuperior.join("|") : "");

			// Actualiza la lista de filtros que no son de los seleccionados en la "jerarquía"
			setTimeout(function() {
				$("#" + IdTabla + " select.filtro-excel:not("+ lstFiltrosXActualizar.join(",") + ")").each(function() {
					...
					// En lugar de actualizar cada filtro tipo Excel encadenados de los que no fueron seleccionados, se agregó al momento de generar el datatable
					// un botón con funcionalidad que en caso de que no haya sido seleccionado, al momento de usarlo haga la actualización y genere el filtro tipo Excel para ser utilizado
					$("#" + $(this).attr("id")).multiselect("destroy").hide();
				})
			}, 250);

			// Finalmente Actualiza la Tabla correspondiente con las opciones [des]seleccionadas
		...
		function cargarWokflow(elemento) {
			// Determina el workflow a cargar
			var NombreTabla = $(elemento).data("id-tabla");
			var Id_Activo = $(elemento).data("id-activo");
			var Id_Area = $("#idareasesion").val();
			var parametrosJson = { accion: "WorkflowGet", Id_Area: Id_Area, NombreTabla: NombreTabla, Id_Activo: Id_Activo };
			// Solo en caso de baja/reubicación
			if($(elemento).data("id-activo-reubicacion")) { parametrosJson.Id_Activo_Reubicacion = $(elemento).data("id-activo-reubicacion"); }
			//console.log("Generar workflow: ");
			//console.log(parametrosJson)

			// Carga la página que contiene el workflow de la administración seleccionada ubicada dentro de la modal de administración correspondiente
			//$("#modal_workflow_" + NombreTabla).load("../controladores/simple_mvc/ActivoController.Class.php", parametrosJson, function (response, status, xhr) {
			$("#areaComunWorkflow").load("../controladores/simple_mvc/ActivoController.Class.php", parametrosJson, function (response, status, xhr) {
				if (status == "error") {
					// Ha ocurrido un error y debe mostrarse
					var msg = "Sorry but there was an error: ";
					mensajesalerta("Informaci&oacute;n", msg + xhr.status + " " + xhr.statusText, "error", "dark");
		...
		function iniciarWorkflow(elemento = null) {
			if(elemento != null) {
				...
				var Id_Activo = $(elemento).data("id-activo");
				var Id_Area = $("#idareasesion").val();
				var parametrosJson = { accion: "AddWorkflow", Origen: "WorkflowAjax", Id_Area: Id_Area, NombreTabla: NombreTabla, Id_Activo: Id_Activo, CveWorkflow: 1, IdUsuario: $("#usuariosesion").val() };
				// Solo en caso de baja/reubicación
				if($(elemento).data("id-activo-reubicacion")) { parametrosJson.Id_Activo_Reubicacion = $(elemento).data("id-activo-reubicacion"); }
				//console.log(parametrosJson);
				...
				$.ajax({
					...
					success: function (resultado) {
						...
						if(respuesta[0].intResultado > 0) {
							mensajesalerta("&Eacute;xito", respuesta[0].strMensaje, "success", "dark");
							var objetoActivoTemp = { NombreTabla: NombreTabla, Id_Activo: respuesta[0].intResultado };
							cargarWokflow(elemento);
						}
						else {
							mensajesalerta("Oh No!", respuesta[0].strMensaje, "error", "dark");
						}
					},
					error: function (response, status, xhr) {
						mensajesalerta("Oh No!", msg + xhr.status + " " + xhr.statusText, "error", "dark");
					}
				});
			}
		}
		...
		function sigaSelectCondicionDeRecepcion(id_activo,id_selector){
			...
			$.ajax({
				type: "POST",
				url: "../class/ajax/sigaCatalogos.ajax.php",
				data: {accion:2, id_activo:id_activo,id_selector:id_selector},
				dataType: "JSON",
				success: function (response) {
					$('#siga_cmb_condicion_recepcion').html(response);
				}
			});
		...
		function sigaVrdAltaActivo(usuariosesion){
			$.ajax({
				...
				data: {accion:2,usuariosesion:usuariosesion},
				dataType: "JSON",
				success: function (response) {
					//console.log(response);
					//alert('Se mando vale');
...
<style>
	...
	/*.barrasLateralesDataTableIzquierdo { width: 25px; height: 100%; background: #FFF; position: absolute; top: 0; left: 0; z-index: 1001; }*/
	.barrasLateralesDataTableDerecho { width: 25px; height: 100%; background: #FFF; position: absolute; top: 0; right: 0; z-index: 1001; }
