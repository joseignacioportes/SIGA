 ...
 .modal-open {
    ...
    <div id="tabla_dinamica_reporte">
		...
		<ul class="nav nav-tabs azulf" role="tablist">
			...
			<li>
			Reporte: <select id="cmbAnual" disabled>
			...
			Tabla INPC: 
							<select id="cmbINPC" disabled>
							  <option <?php if ($Anio < 2018) echo "selected" ?> value="2017">2017</option>
			...
			<button type="button" class="btn chs" id="btnBuscar">Buscar</button>
                    ...
                    <thead>
                      <tr>
					    <th style="<?php echo $style_size_table;?>">Componentes</th>
						...
						<th style="<?php echo $style_size_table;?>">Depreciacion antes de cambio contable </th>
						<th style="<?php echo $style_size_table;?>">Depreciación Total</th>
                    ...
                    <tbody>
					 ...
					 { 
					  ...
					  $aniosArr = $obj->obtenFiscalD4($activos[$j]["AF_BC"],$Anio,1,$Anual,$Mes,0,$inpcperiodo);
					  $hijos = "";
                      ...
                      <tr>
						...
						<td style="<?php echo $style_size_table;?>"><?php echo $aniosArr[0]["MOIAntes"] ?></td>
						
						<td style="<?php echo $style_size_table;?>"><?php echo number_format($aniosArr[0]["MOIAntes"]+$aniosArr[0]["depreciacionacumuladafinperiodo"],2)?></td>
					  ...
					  if ($aniosArr[0]["TieneHijos"] > 0 )
					  {
						...
						for ($k=0; $k < count($aniosArr2); $k++)
						{
						   ...
						   // print_r($hijosArr);
						   //echo "<br>MOIS=".$hijosArr[0]["MOI"].",".$aniosArr2[$k]["AF_BC"];
					  ?>
					  <tr>
						...
						<td style="<?php echo $style_size_table;?>"><?php echo number_format($aniosArr[0]["MOIAntes"] ,2) ?></td>
						<td style="<?php echo $style_size_table;?>"><?php echo ($aniosArr[0]["MOIAntes"]+$hijosArr[0]["depreciacionacumuladafinperiodo"])?></td>
                      </tr>
...
<script>
    ...
    $('#example').DataTable({
	  ...
	  /*"footerCallback": function( tfoot, data, start, end, display ) {
		...
		$( api.column( 8 , {page: 'current'}).footer() ).html(
			api.column( 8 ,{page: 'current'}).data().reduce( function ( a, b ) {
				return a + b;
			}, 0 )
		);
	  },*/
 ...
 $("#btnBuscar").click(function(e){
	 //e.preventDefault();
	 var bajas = $("#cmbBajas").val();
	 if ($("#cmbAnual").val() == 0)
	 loadOptionBandejas('reportecontabilidad11.php?Id_Menu=5&Id_Submenu=7&Anio='+$("#cmbAnioFiltro").val()+'&Anual='+$("#cmbAnual").val()+'&Bajas='+bajas+'&INPCPeriodo='+$("#cmbINPC").val(),'contenedorprincipalactivofijo','3016'); 
	 else
	 loadOptionBandejas('reportecontabilidad1.php?Id_Menu=5&Id_Submenu=7&Anio='+$("#cmbAnioFiltro").val()+'&Anual='+$("#cmbAnual").val()+'&Mes='+$("#cmbMesFiltro").val()+'&Bajas='+bajas+'&INPCPeriodo='+$("#cmbINPC").val(),'contenedorprincipalactivofijo','3016'); 
