	...
	$("#select-activos").change(function() {
      ...
      <div class="row">
        ...
        <ul class="nav nav-tabs azulf" role="tablist">
          <li role="presentation" onclick="tab_active(1)" class="active"><a href="#inventario" aria-controls="inventario" role="tab" data-toggle="tab" id="tabNuevas">Solicitud Nueva</a></li>
          <li role="presentation" onclick="tab_active(2)"><a href="#altas" aria-controls="altas" role="tab" data-toggle="tab"  id="tabSeguimiento">Solicitado</a></li>
          <li role="presentation" onclick="tab_active(3)"><a href="#historico" aria-controls="cerrado" role="tab" data-toggle="tab"  id="tabCerrado">Devuelto</a></li>
                  ...
                  <table id="tablaSeguimiento" class="table table-bordered table-striped table-chs" width="100%">
                    <thead>
                      <tr>
						...
						<th>Gestor</th>
                        <th><?php echo $Espacios; ?>Fecha Prestamo / Devolución<?php echo $Espacios; ?></th>
                        <th><?php echo $Espacios; ?>Descripción&nbsp;<?php echo $Espacios; ?></th>
                        <th>Activo Fijo</th>
                      </tr>
                    </thead>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body nopsides">
            ...
            <ul class="nav nav-tabs azuldef" role="tablist">
              ...
              <li role="presentation" ><a id="tab_activos" href="#activos" aria-controls="activos" role="tab" data-toggle="tab">Activos Disponibles</a></li>
              <li role="presentation"><a  href="#cerrar" id="tab_cerrar_ticket" aria-controls="cerrar" role="tab" data-toggle="tab">Cerrar préstamo</a></li>
			  <li role="presentation"><a  href="#devolver" id="tab_devolver" aria-controls="cerrar" role="tab" data-toggle="tab" style="display:none">Devolución</a></li>
            </ul>
             ...
             <div role="tabpanel" class="tab-pane active" id="datos">
               ...
               <div class="col-md-12 col-md-offset-1">
                ...
                <div class="modal-footer">
                  <button type="button" id="btnAsignar" class="btn chs">Siguiente</button>
                </div>
              </div>
            ...
            <div role="tabpanel" class="tab-pane" id="activos">
               <div class="box-body">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn chs" id="btnAsignar2">Asignar Activo</button>
                </div>
              ...
              <div role="tabpanel" class="tab-pane" id="cerrar">
               ...
               <div class="modal-footer">
                  <button type="button" class="btn chs" id="botonCerrar">Realizar préstamo</button>
               </div>
              </div>
			  ...
			  <div role="tabpanel" class="tab-pane" id="devolver">
               ...
               <div class="modal-footer">
                  <button type="button" class="btn chs" id="botonDevolver">Devolver activo</button>
               </div>
              </div>
...
<script>
  ...
  $('.datepicker').datepicker({ 
	...
	function imagenes_chat(){
		...
		$('#upload_adjuntos_chat').fileinput({
			uploadUrl: "../Archivos/upload.php?carpeta=../Archivos/Archivos-Chat/",
			uploadAsync: true,
			showUpload: true,
			showRemove: false,
			showZoom: false,
		...
		//Cargar Archivo
		$('#upload_adjuntos_chat').on('fileuploaded', function (event, data, previewId, index) {
			var form = data.form, files = data.files, extra = data.extra,
				response = data.response, reader = data.reader;
			$('#url_documentos_adjuntos_chat').val(response.initialPreviewConfig[0].caption);
			$("#botonEnviar").html("Enviar Archivo");
		});
		
		//Eliminar Archivo
		$('#upload_adjuntos_chat').on('filereset', function (event) {
			$('#url_documentos_adjuntos_chat').val("");
...
$(document).ready(function(){
		...
		"columns": [
			...
			{	
				"data": function (obj) {
					...
					seguimiento += '<a target="_blank" href="https://apps2.hospitalsatelite.com/siga/controladores/activos/siga_solicitud_prestamos/Reporte-Prestamo.php?Id_Solicitud='+obj.Id_Solicitud+'" title="pdf"><span><i class="fa fa-file-pdf-o" aria-hidden="true"></i></span></a>';
	...
	$('#tablaCierre').DataTable({
		...
		"columns": [
			...
			{	
				"data": function (obj) {
					var seguimiento = '';
					seguimiento += '<a href="#" data-toggle="modal" data-target="#seguimientoPrestamos" onclick="pasarvalores('+obj.Id_Solicitud+', 4)"><span><i class="fa fa-pencil" aria-hidden="true"></i></span></a>';
					seguimiento += '<a target="_blank" href="https://apps2.hospitalsatelite.com/siga/controladores/activos/siga_solicitud_prestamos/Reporte-Prestamo.php?Id_Solicitud='+obj.Id_Solicitud+'" title="pdf"><span><i class="fa fa-file-pdf-o" aria-hidden="true"></i></span></a>';
					return seguimiento;
				}
			},
	...
	pasarvalores=function(id, Estatus_Proceso) {
		...
		if(Estatus_Proceso=="3")
		{
			...
			$("#TituloCierre").prop('disabled', false);
			$("#botonCerrar").html("Actualizar Préstamo");
			$("#tab_devolver").show();
		}
...
function limpiarcampos()
	...
	$("#botonCerrar").click(function(e){
		...
		//Area de sesion
		//var Id_Area=$("#idareasesion").val();
		var TituloCierre=$.trim($("#textEstadoEquipo").val());
		var Accesorios_Cierre=$.trim($("#textAccesorios").val());
		var ComentarioCierre = $("#textComentarios").val();
		var Estatus_Proceso = $("#hddEstatus_Proceso").val();
		var Id_Activo = $("#txt_Id_Activo").val();
		...
		if (Estatus_Proceso == '') {
			Agregar = false; 
			mensaje_error += " -El préstamo debe estar asignado a un gestor para ser cerrado<br />";
			$("#Solucion").focus();
		}
		
		if (TituloCierre.length <= 0) {
			Agregar = false; 
			mensaje_error += " -El Estado del Equipo de cierre no puede ser vacio<br />";
			$("#TituloCierre").focus();
		}
		...
		if(Agregar)
		{
			...
			strDatos += "&Accesorios_Cierre="+Accesorios_Cierre;
			...
			if(Id_Solicitud_Correo!=""){
				$.ajax({
					type: "POST",
					url: "../fachadas/activos/siga_solicitud_prestamos/Siga_solicitud_prestamosCorreo.Class.php",        
					async: false,
					data: {
						tipocorreo:"cerrar_gestor",
						nombre:"Javier",
						telefono:"5514284951",
	...
	$("#btnAsignar2").click(function (e){
		
		if ( $("#txt_Id_Activo").val() == "")
		{
			mensajesalerta("Oh No!", "Debe elegir un activo para realizar el préstamo.", "error", "dark");
		    $("#tab_cerrar_ticket").hide();
		}
		else
		{
	...
	$("#botonDevolver").click(function(e){
		...
		//Area de sesion
		//var Id_Area=$("#idareasesion").val();
		var ComentarioEntrega = $("#textComentariosEntrega").val();
		var Fecha_Entrega=$.trim($("#Fecha_Entrega").val());
		var Estatus_Proceso = $("#hddEstatus_Proceso").val();
		
		if (Estatus_Proceso != 3 && Estatus_Proceso != 4) {
			Agregar = false; 
			mensaje_error += " -El préstamo debe estar cerrado  para ser entregado<br />";
			$("#textComentariosEntrega").focus();
		}
		
		if (ComentarioEntrega.length <= 0) {
			Agregar = false; 
			mensaje_error += " -El Comentario de entrega no puede ser vacio<br />";
			$("#ComentarioEntrega").focus();
		}
		if (Fecha_Entrega.length <= 0) 
		{
			Agregar = false; 
			mensaje_error += " -Falta agregar la fecha de entrega real<br />";
			$("#Fecha_Entrega").focus();
		}			
		if (!Agregar) {
			mensajesalerta("Informaci&oacute;n", mensaje_error, "", "dark");			
		...
		if(Agregar)
		{
			...
			strDatos = "Id_Solicitud="+Id_Solicitud; 
			strDatos += "&ComentarioEntrega="+ComentarioEntrega;
			strDatos += "&Fecha_Entrega="+Fecha_Entrega;
